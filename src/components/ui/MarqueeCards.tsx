...
export const MarqueeCards = () => {
  const getContractIcon = (name: string) => {
    ...
    if (lower.includes("token") && lower.includes("erc20")) {
      return <Coins className="w-7 h-7 text-orange-300" />;
    }
    if (lower.includes("token") && lower.includes("erc721")) {
      return <Shield className="w-7 h-7 text-orange-400" />;
    }
    if (lower.includes("token") && lower.includes("erc1155")) {
      return <Layers className="w-7 h-7 text-orange-300" />;
    }
    if (lower.includes("marketplace")) {
      return <Globe className="w-7 h-7 text-orange-400" />;
    }
    if (lower.includes("vote") || lower.includes("split")) {
      return <Users className="w-7 h-7 text-orange-300" />;
    }
    if (lower.includes("account")) {
      return <Shield className="w-7 h-7 text-orange-400" />;
    }
    return <Code className="w-7 h-7 text-orange-200" />;
  };
      ...
      <div className="flex animate-marquee space-x-6 hover:pause">
        {[...contracts, ...contracts].map((contract, index) => (
          <motion.div
            ...
            className="group relative flex-shrink-0 w-96 h-64 rounded-2xl bg-gradient-to-br from-black/50 to-orange-950/60 backdrop-blur-sm border border-orange-700/50 hover:border-orange-400 p-8 transition-all duration-300 cursor-pointer hover:shadow-xl hover:shadow-orange-400/30"
          >
            {/* Category Badge */}
            <div className="absolute top-4 right-4">
              <span className="px-3 py-1.5 text-xs font-medium bg-orange-800/40 text-orange-200 rounded-full border border-orange-600/30">
            ...
            <div className="flex items-start gap-4 mb-4">
              <motion.div 
                className="p-3 rounded-xl bg-orange-900/40 group-hover:bg-orange-600/25 transition-colors border border-orange-700/30"
                whileHover={{ rotate: 5 }}
              >
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-orange-300">v{contract.version}</p>
              </div>
            </div>

            <p className="text-base text-orange-100 leading-relaxed line-clamp-3">
              {contract.description}
            </p>

            {/* Hover overlay */}
            <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-orange-700/15 to-orange-900/15 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          </motion.div>
        ))}
      </div>
