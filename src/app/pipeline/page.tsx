...
const nodeTypes: Record<string, NodeType> = {
  ...
  deploy: { icon: GitBranch, color: "bg-blue-800", label: "Deploy Core Testnet" },
    deployMainnet: { icon: GitBranch, color: "bg-red-600", label: "Deploy Core Mainnet" },
...
const templates: Record<string, Template> = {
  basic: {
    ...
    nodes: [
      ...
      { id: "3", type: "deploy", x: 510, y: 100, config: { network: "core-testnet", gasLimit: "2000000" } },
    ],
    connections: [
      { from: "1", to: "2" },
      { from: "2", to: "3" },
  ...
  production: {
    ...
    nodes: [
      ...
      { id: "5", type: "deploy", x: 650, y: 50, config: { network: "core-testnet" } },
      { id: "6", type: "verify", x: 800, y: 50, config: { explorer: "core-scan" } },
      { id: "7", type: "condition", x: 650, y: 200, config: { condition: "testnet_success" } },
      { id: "8", type: "deployMainnet", x: 800, y: 200, config: { network: "core-mainnet" } },
    ],
    connections: [
      { from: "1", to: "2" },
      { from: "2", to: "3" },
  ...
  multiContract: {
    ...
    nodes: [
      ...
      { id: "5", type: "deploy", x: 550, y: 50, config: { network: "core-testnet", contract: "Token" } },
      { id: "6", type: "deploy", x: 550, y: 150, config: { network: "core-testnet", contract: "Pool" } },
    ],
    connections: [
      { from: "1", to: "3" },
      { from: "2", to: "3" },
...
const deployToTestnet = async (
  ...
  logger(`🚀 Deploying to Core Testnet`, "info")
  ...
  try {
    ...
    console.log("Core testnet deploy result:", deploymentResult)

    if (deploymentResult && deploymentResult.contractAddress) {
      logger(`✅ Contract deployed to Core testnet successfully`, "success")
      ...
      logger(`🌐 Network: Core Testnet`, "info")
      return true
    } else {
      logger(`❌ Core testnet deployment failed: ${deploymentResult?.error || "Unknown error"}`, "error")
      return false
    }
  } catch (error) {
    logger(`❌ Core testnet deployment error: ${error}`, "error")
    return false
  }
}
...
const deployToMainnet = async (
  ...
  logger(`🚨 CORE MAINNET DEPLOYMENT INITIATED`, "info")
  logger(`⚠️ Network: Core Mainnet`, "info")
  ...
  if (!isConnected) {
    logger(`❌ Wallet not connected. Core mainnet deployment requires wallet connection.`, "error")
    return false
  }

  if (!compilationResult) {
    logger(`❌ No compilation result found. Core mainnet deployment requires compilation.`, "error")
    return false
  }

  // Show confirmation dialog for mainnet
  const confirmed = window.confirm(
    `🚨 CORE MAINNET DEPLOYMENT CONFIRMATION\n\n` +
    `You are about to deploy to Core Mainnet.\n` +
    `Network: ${node.config.network}\n` +
    `Gas Limit: ${node.config.gasLimit}\n` +
    `Gas Price: ${node.config.gasPrice}\n\n` +
    `This will use real CORE tokens. Are you sure you want to proceed?`
  )

  if (!confirmed) {
    logger(`❌ Core mainnet deployment cancelled by user`, "error")
    return false
  }

  try {
    logger(`🔄 Processing Core mainnet deployment...`, "info")
    ...
    logger(`✅ Core mainnet deployment simulation completed`, "success")
    ...
    logger(`❌ Core mainnet deployment error: ${error}`, "error")
    return false
  }
}
...
const handleInputChange = (key: string, value: string | number | boolean) => {
    ...
    <div>
      ...
      >
        {node.type === "deploy" ? (
          ...
          <option value="core-mainnet">Core Mainnet</option>
        )}
      </select>
      {node.type === "deployMainnet" && (
        <p className="text-xs text-red-400 mt-1">⚠️ Core mainnet deployment - use with caution</p>
      )}
    </div>
    <div>
        ...
        <p className="text-xs text-gray-400 mt-1">Number of block confirmations required</p>
            ...
            {node.type === "verify" && (
              <div>
                ...
                <select
                  value={node.config.explorer || "core-scan"}
                  onChange={(e) => handleInputChange("explorer", e.target.value)}
                  className="w-full p-2 border border-gray-600 rounded bg-gray-700 text-gray-300"
                >
                  <option value="core-scan">Core Scan</option>
 ...
 const renderWalletButton = () => {
  ...
  <div className="p-4 border-b border-gray-800 flex-shrink-0 mt-26">
    <h3 className="text-lg font-semibold text-white">Core Pipeline</h3>
  </div>
          ...
          >
            ...
            {nodes.map((node) => {
              ...
              return (
                <div
                  key={node.id}
                  className={`absolute bg-gray-800 border-2 rounded-lg shadow-xl p-3 cursor-pointer hover:shadow-2xl hover:shadow-orange-500/20 transition-all duration-200 ${
                    selectedNode === node.id ? "border-orange-500 shadow-orange-500/30" : "border-gray-700"
                  }`}
...
{executionLogs.length > 0 && (
  <div className="w-80 bg-black border-l border-gray-800 flex flex-col h-full max-h-screen">
    <div className="p-4 border-b border-gray-800 flex-shrink-0">
      <h3 className="text-orange-400 font-semibold">Execution Logs</h3>
    </div>
    <div className="flex-1 p-4 overflow-y-auto font-mono text-sm min-h-0">
      {executionLogs.map((log, index) => (
        ...
        >
          <span className="text-orange-500">[{log.timestamp}]</span> {log.message}
        </div>
      ))}
    </div>
  </div>
