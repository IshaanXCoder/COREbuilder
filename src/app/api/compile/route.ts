...
// Ensure resolc.wasm exists in Next server vendor-chunks before loading resolc
const ensureResolcWasm = () => {
  try {
    const vendorDir = join(process.cwd(), '.next/server/vendor-chunks')
    const wasmTarget = join(vendorDir, 'resolc.wasm')
    if (!existsSync(wasmTarget)) {
      const wasmSource = join(process.cwd(), 'node_modules/@parity/resolc/dist/resolc/resolc.wasm')
      // Lazily create directory and copy
      try {
        // @ts-ignore - node 16 doesn't have recursive type
        require('fs').mkdirSync(vendorDir, { recursive: true })
      } catch {}
      require('fs').copyFileSync(wasmSource, wasmTarget)
    }
  } catch (e) {
    console.warn('Could not ensure resolc.wasm in vendor-chunks:', e)
  }
}
